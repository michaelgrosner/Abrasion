...
def usage():
	...
	SAMPLE USAGE:
	python googleize.py "search phrase" [option name]=[option value]
	
	Or within a python script:
	import googleize
	'Number of results', [link list], [title list] =
		googleize.googleize([search string], [option name]=[option value])
	...
	--type=       Type of search. Defaults to `search`. Also supported:
		 `news`      Google News
		 `shopping`  Google Shopping (although their links are quite uninteresting and no
                   Google Shopping specific options are implemented yet)
     `blogs`     Search blogs
	...
	python googleize.py "Android vs. iPhone" --nresults=100 --timing=range --datestart=10/20/2009
